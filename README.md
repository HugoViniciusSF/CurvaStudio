...
- Guia **Looping 3D** com desenho e edição de coordenadas, câmera rotacionável e modos de partícula presa ou solta, com desprendimento e voo sob a gravidade.
...
O modelo inicial tem raio de **2 m**, altura de partida de **6 m**, profundidade de **1,2 m**, massa de **12 kg**, gravidade de **9,81 m/s²**, velocidade inicial nula e partícula **Presa**.
...
| Vínculo com a pista | Presa ou solta |

1. Use **Partida a 3R** ou **Partida a 1,4R** para comparar energias iniciais. Ambos usam `g = 9,81 m/s²` e `v₀ = 0`, preservando raio, profundidade, massa e vínculo. **Testar 2,4R** e **Testar 2,5R** preparam a comparação do contato: também zeram a profundidade e selecionam **Solta**.
...
Escolha o **Vínculo com a pista** nas condições físicas:

- **Presa**: a guia pode empurrar ou puxar a partícula. Ela pode completar um looping partindo de `2,4R`; uma reação normal negativa indica que a guia precisa puxá-la.
- **Solta**: a pista só empurra no sentido de sustentação. Quando a continuação exigiria uma reação negativa, a partícula se desprende com a velocidade tangente à pista e segue em voo sob a gravidade. A cena marca o desprendimento e mostra o rastro do voo.

O modelo não inclui atrito, rotação nem resistência do ar. Em cada segmento da pista, usa a solução de aceleração tangencial constante `a = −g Δy/Δs` e a relação energética `v² = v₀² + 2g(y₀ − y)`. No voo, usa as equações do movimento sob gravidade constante em `(x, y, z)`. A conservação de `Em = Ec + mgy` faz parte do método, independente da integração RK4 do Laboratório.

Para um looping **plano e circular**, com partida do repouso, a condição mínima para manter contato é `H = 2,5R`: no topo, `v² = gR` e `N = 0`. Com `R = 1 m` e `H = 2,4 m`, a partícula solta se desprende antes do topo. O código calcula a reação local a partir da velocidade e da curvatura, sem impor um corte pela altura. Com profundidade variável, `R` é o raio da projeção em `xy`, e a regra plana não se aplica diretamente. A pista espacial adota uma orientação de sustentação e uma guia lateral ideais.

Se a energia for insuficiente para vencer uma subida, a partícula pode retornar enquanto mantiver contato. O ensaio encerra ao atingir uma extremidade, permanecer em repouso ou sair da região de observação do voo. Não há colisão com o solo nem novo contato após o desprendimento, mesmo que o voo cruze a pista.

O intervalo das amostras tem padrão de **0,05 s** e opções **0,025; 0,05; 0,1; 0,25; 0,5; 1 e 2 s**. Mudar o intervalo seleciona novamente os estados calculados. O início, o desprendimento, o encerramento e os avanços manuais são preservados. A aba **Dados 3D** mostra as últimas oito amostras, incluindo a normal e a fase do movimento; **Exportar CSV 3D** salva toda a série selecionada em `curva-studio-looping-<data-hora>.csv`:
...
`distancia_m` representa a coordenada `s` desde o início da pista; diminui no retorno e fica constante em voo. Não é a distância total percorrida. `velocidade_m_s` tem sinal na pista e representa a rapidez em voo; `vx_m_s`, `vy_m_s` e `vz_m_s` indicam a direção espacial. `forca_normal_N` é a componente de sustentação, zero em voo. `fase` identifica `pista`, `voo` ou `voo_encerrado`.

O CSV usa UTF-8, vírgula entre colunas, ponto decimal e até 12 algarismos significativos nos valores numéricos. Desenhar, editar pontos, gerar a trajetória ou alterar as condições físicas, inclusive o vínculo, reinicia somente os registros desta guia. Recarregar a página descarta os ensaios das duas guias.
...
Os testes incluem soluções analíticas, conservação de energia, dissipação, velocidades de até 1.000 m/s, pausa, amostragem e exportação. As suítes do looping também verificam trajetórias em três dimensões, interseções, inversões, o limiar de contato em `2,5R`, desprendimento, voo balístico, dados do CSV e preservação do estado quando a guia fica inativa.
...
| [looping3d.ts](src/simulacao/looping3d.ts) | Geometria espacial, movimento na pista, desprendimento e voo. |
| [contatoLooping3d.ts](src/simulacao/contatoLooping3d.ts) | Curvatura, orientação de apoio, normal e localização da perda de contato. |
